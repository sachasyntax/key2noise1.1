...
last_audio_block = np.zeros(1024, dtype=np.float32)
...
rev_bufs = [
    ...
    np.zeros(1700, dtype=np.float32),
    np.zeros(2300, dtype=np.float32),
    np.zeros(3100, dtype=np.float32),
]
...
# reverb
def reverb(signal, mix=0.25, fb=0.55):
    global rev_bufs, rev_pos, rev_bright 
    ...
    for i in range(len(sig)):
        ...
        for n in range(4):
            ...
            y = buf[pos]
            lp = (1 - rev_bright) * lp + rev_bright * y  #damping

            buf[pos] = s + lp * fb
            rev_pos[n] = (pos + 1) % len(buf)

            acc += y

        # diffusion
        wet[i] = np.tanh(acc * 0.6)

    return sig * (1.0 - mix) + wet * mix

# centroid mapping function
def map_centroid(signal, sr=44100, min_val=0.1, max_val=0.7, alpha=0.05):

    # centroid
    fft_mag = np.abs(np.fft.rfft(signal))
    freqs = np.fft.rfftfreq(len(signal), 1/sr)
    centroid = np.sum(freqs * fft_mag) / (np.sum(fft_mag) + 1e-6)
    centroid_norm = np.clip(centroid / (sr/2), 0.0, 1.0) 
    ...
    # mapping inverso
    target_val = max_val - (max_val - min_val) * centroid_norm

    # smoothing 
    if not hasattr(map_centroid, "_state"):
        map_centroid._state = target_val
    map_centroid._state += alpha * (target_val - map_centroid._state)

    return map_centroid._state

# differential mapping
def diff_map(signal, clip_min=0.0, clip_max=0.2, lerp=0.05):
    d = np.mean(np.abs(np.diff(signal)))
    d = np.sqrt(d)
    d_norm = linlin(d, clip_min, clip_max, 0.0, 1.0)
    d_norm = np.clip(d_norm, 0.0, 1.0)

    # smoothing 
    if not hasattr(diff_map, "_state"):
        diff_map._state = d_norm
    diff_map._state += (d_norm - diff_map._state) * lerp
    return diff_map._state

# spectroid mapping
def spectral_map(signal, clip_min=0.0, clip_max=0.2, lerp=0.05):
    fft_mag = np.abs(np.fft.rfft(signal))
    c = np.mean(fft_mag[len(fft_mag)//2:])
    c = np.sqrt(c)
    c_norm = linlin(c, clip_min, clip_max, 0.0, 1.0)
    c_norm = np.clip(c_norm, 0.0, 1.0)

    # smoothing 
    if not hasattr(spectral_map, "_state"):
        spectral_map._state = c_norm
    spectral_map._state += (c_norm - spectral_map._state) * lerp
    return spectral_map._state
...
def process_direct(raw):
    ...
    # add raw input
    raw_factor = 0.95
    e = np.clip(energy[1]**0.5 * 3, 0.0, 1.0)
    g = linlin(e, 0.0, 1.0, 0.25, 0.5)
    r = np.tanh(raw[:n] * raw_factor * 1.5)
    ...
    # feedback
    fb_amt = map_centroid(direct_memory[:n]) 
    direct_memory[:n] *= dir_factor
    ...
    # interp
    delay_f = linlin(energy[3]**1.5, 0.0, 1.0, 0.01, 3.0)
    if not hasattr(process_direct, "_delay_state"):
        process_direct._delay_state = delay_f
    process_direct._delay_state += (
        delay_f - process_direct._delay_state
    ) * 0.07
    delay_f = process_direct._delay_state
    idx = np.arange(n)
    i0 = (idx - delay_f).astype(int) % n
    i1 = (i0 + 1) % n
    frac = delay_f - np.floor(delay_f)
    direct_memory[:n] = (
        (1 - frac) * direct_memory[i0] +
        frac * direct_memory[i1]
    )
    ...
    # lowpass
    d = diff_map(direct_memory[:n])
    c = spectral_map(direct_memory[:n])
    dir_cutoff = linlin(d, 0.0, 1.0, 0.09, 0.4)   
    comb_cutoff = linlin(c, 0.0, 1.0, 0.01, 0.28) #ex 0.2

    direct_memory[:n] = lowpass(direct_memory[:n], dir_cutoff)
    comb_memory[:n] = lowpass(comb_memory[:n], comb_cutoff)

    # smear
    smear = linlin(energy[1], 0.0, 1.0, 0.05, 0.2) # ex speed, 0.2
    direct_memory[1:n] = (
...
def process_percussive(frames):
    ...
    # lowpass
    y = np.sqrt(np.mean(np.abs(np.diff(out))))
    y = y*0.6 + energy*0.4
    y = np.clip(y, 0.0, 0.2)

    cutoff = linlin(y, 0.0, 0.2, 0.09, 0.3)   
...
def audio_callback(outdata, frames, t, status):
    global acc_read_pos, last_audio_block
    global rev_mix, rev_fb
    ...
    # hold if active
    if not got_data:
        if direct_active or acc_active:
            out[:] = last_audio_block[:frames]
        else:
            out[:] = 0.0  # silence
    else:
        last_audio_block[:frames] = out
    ...
    if acc_active:
        ...
        out += acc_block[:frames]

    # perc
    if perc_active and perc_pool.active:
        perc = process_percussive(frames)
        if perc is not None:
            out += perc_mix * perc[:frames]

    # reverb
    out = reverb(out, mix=rev_mix, fb=rev_fb)

    # tanh + limiter
    tanh_amp = 1.3
    out *= tanh_amp
    outdata[:, 0] = np.tanh(out)
...
# log box
log_box = tk.Text(root, bg="black", fg="#00FF00", font=("Consolas", 20))
...
def stop_pool1():
    ...
    with audio_queue.mutex:
        audio_queue.queue.clear()  # dequeue
    with lock:
        accumulator[:] = 0
        direct_memory[:] = 0
        fb_buf_acc.clear()
        fb_buf_direct.clear()
        global last_audio_block
        last_audio_block[:] = 0.0  # reset 
...
# pulsanti frame
frame_audio = tk.Frame(root, bg="#00FF00", bd=2, relief="ridge")
